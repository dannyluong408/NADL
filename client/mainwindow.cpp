...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->chat_input, SIGNAL(returnPressed()), ui->chat_send,
          SIGNAL(clicked()));
...
void MainWindow::moveDown() {
  ...
  if (channel[z]->history_list->size() != 9) {
    if (channel[z]->history_list->isEmpty()) {  // Checks if list is empty
      ...
      size = channel[z]->history_list->size();  // Sets size to list size if
                                                // there has not been 10 entries
                                                // yet
    }
  }
  ...
  if (channel[z]->current_history < size && size != 1) {
    ...
    ui->chat_input->insert(
        (QString)channel[z]->history_list->at(channel[z]->current_history));
    ui->chat_input->selectAll();
    ui->chat_input->end(true);
    upDown = true;
  }

  else if (channel[z]->history_list->size() == 1 &&
           channel[z]->current_history == 0) {
    // qDebug() << "2";
    ui->chat_input->clear();
    ui->chat_input->setFocus();
    ui->chat_input->insert(
  ...
  for (int x = 0; x < channel[channel_id]->n_games; ++x) {
    if (channel[channel_id]->games[x].n_users) {
      ...
      if (channel[channel_id]->games[x].start_time) {
        ...
        snprintf(str, sizeof(str), "%s (%s): %im",
                 channel[channel_id]->games[x].game_name,
                 channel[channel_id]->games[x].host_name, minutes);
      } else {
        snprintf(str, sizeof(str), "%s (%s): %i/%i",
                 channel[channel_id]->games[x].game_name,
                 channel[channel_id]->games[x].host_name,
                 channel[channel_id]->games[x].n_users,
                 channel[channel_id]->games[x].n_max);
      }
...
void MainWindow::redraw_rank_list(const uint16_t channel_id) {
  ...
  for (uint32_t x = 0; x < 100; ++x) {
    if (channel[channel_id]->rankings[x][0] != 0) {
      char str[1000];
      snprintf(str, sizeof(str), "%u: %s - %u", x + 1,
               channel[channel_id]->rankings[x],
               channel[channel_id]->ratings[x]);
...
Widget_Pointers *MainWindow::createTab(QWidget *tab_x) {
  ...
  verticalLayout_x->addLayout(horizontalLayout_y);  // GamesLayout add
                                                    // rank/games
  ...
  listWidget_x->setSizePolicy(
      QSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed));
  listWidget_y->setSizePolicy(
      QSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed));
  user_list_x->setSizePolicy(
  ...
  connect(listWidget_x, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
...
void MainWindow::manual_connect() {
  if (connection_attempted == 2) {
    ...
    QString styledString =
        "<span style=\" font-size:10pt; font-weight:600; color:#D21B1B;\" > ";
  ...
  if (!connection_attempted) {
    ...
    if (!login_sent) {
      ...
      if (use_token) {
        ...
        ui->status_browser->append(
            "Adding -save after your password will automatically login in the "
            "future.");
      }
    }
      ...
      while (remaining_length) {
        if (position[0] >= 200) {  // disconnect msg
          if (position[0] == SERVER_MESSAGE_DISCONNECT_SERVER_SHUTDOWN) {
            const char *dcmsg =
                "Disconnected from server. Server is shutting down. Please "
                "restart your client.";
            for (uint x = 0; x < n_channels; ++x) {
            ...
            ui->status_browser->append(dcmsg);
                     ...
                     SERVER_MESSAGE_DISCONNECT_USER_LOGGED_IN_ELSEWHERE) {
            const char *dcmsg =
                "Disconnected from server. User logged in elsewhere.";
            for (uint x = 0; x < n_channels; ++x) {
              channel[x]->channel_data->browsers->append(dcmsg);
            }
            ui->status_browser->append(dcmsg);
          } else
            printf("Disconnected from server. Error code: %i\n", position[0]);
          fflush(stdout);
          buffer_pos = 0;

          if (position[0] != SERVER_MESSAGE_DISCONNECT_USER_LOGGED_IN_ELSEWHERE)
            connection_attempted = 0;
          else
          ...
          switch (position[0]) {
            ...
            case SERVER_MESSAGE_BAD_AUTH_TOKEN:
              remove("user");
              ui->status_browser->append(
                  "Server reported bad or expired token. Log in manually by "
                  "using /login (username) (password).");
            ...
            case SERVER_MESSAGE_CHANNEL_UPDATE_GAMEINFO: {
              if (remaining_length > sizeof(game_state)) {
                ...
                for (uint32_t x = 0; x < n_channels; ++x) {
                  if (channel[x] &&
                      channel[x]->channel_id == new_state.league_id) {
                    if (new_state.game_id >= channel[x]->n_games) {
                      game_state *ptr = (game_state *)malloc(
                          sizeof(game_state) * (new_state.game_id + 1));
                      memset(ptr, 0,
                             sizeof(game_state) * (new_state.game_id + 1));
                      memcpy(ptr, channel[x]->games,
                             sizeof(game_state) * channel[x]->n_games);
              ...
              if (remaining_length > sizeof(rank_state)) {
                ...
                for (uint16_t x = 0; x < n_channels; ++x) {
                  if (channel[x]->channel_id == new_state.channel) {
                    ...
                    strncpy(channel[x]->rankings[new_state.position],
                            new_state.name, 64);
                    redraw_rank_list(x);
                    x = n_channels;
                  }
            ...
            case SERVER_MESSAGE_CHAT_MESSAGE: {
              if (remaining_length > sizeof(chat_message_header)) {
                ...
                if (remaining_length - 1 - sizeof(header) >=
                    header.msg_length) {
                  char msgbuffer[2000];
                  memcpy(msgbuffer, &position[1 + sizeof(header)],
                         header.msg_length);
                  ...
                  for (uint8_t x = 0; x < n_channels; ++x) {
                    if (channel[x] &&
                        (channel[x]->channel_id == header.channel ||
                         header.msg_flags & MSG_FLAG_PRINTALL)) {
            ...
            case SERVER_MESSAGE_CONNECT:
              if (remaining_length > sizeof(server_connect_msg)) {
                ...
                // this message is currently irrelevant. figure out a way to
                // handle client updates and it will become relevant.
            ...
            case SERVER_MESSAGE_CHANNEL_REMOVE_USER:
              if (remaining_length > sizeof(chat_user)) {
                ...
                for (uint x = 0; x < n_channels; ++x) {
                  if (channel[x] &&
                      channel[x]->channel_id == user_update.channel) {
                    ...
                    for (int y = 0; y < channel[x]->n_users; ++y) {
                      if (channel[x]->users[y].user_id == user_update.user_id) {
                        ...
                        channel[x]->users[y] =
                            channel[x]->users[channel[x]->n_users];
                        y = channel[x]->n_users;
                      }
                    }
                    ...
                    channel[x]->channel_data->user_lists->sortItems(
                        Qt::AscendingOrder);
                    x = n_channels;
                  }
                }
            ...
            case SERVER_MESSAGE_CHANNEL_ADD_USER:
              if (remaining_length > sizeof(chat_user)) {
                ...
                for (uint x = 0; x < n_channels; ++x) {
                  if (channel[x] &&
                      channel[x]->channel_id == user_update.channel_id) {
                    user_info *ptr = (user_info *)malloc(
                        sizeof(user_info) * (channel[x]->n_users + 1));
                    memcpy(ptr, channel[x]->users,
                           sizeof(user_info) * channel[x]->n_users);
                    ...
                    *channel[x]->completer_list = empty;
                    channel[x]->channel_data->user_lists->clear();
                    // i dont actually remember why we're clearing this and
                    // readding everyone
                    for (int y = 0; y < channel[x]->n_users; ++y) {
                      const char *username = channel[x]->users[y].username;
                      channel[x]->channel_data->user_lists->addItem(username);
            ...
            case SERVER_MESSAGE_CHANNEL_UPDATE_USER:
              if (remaining_length > sizeof(user_info)) {
                ...
                for (uint x = 0; x < n_channels; ++x) {
                  if (channel[x] &&
                      channel[x]->channel_id == update.channel_id) {
                    for (int y = 0; y < channel[x]->n_users; ++y) {
                      if (channel[x]->users[y].user_id == update.user_id) {
                        ...
                        for (int z = 0; z < channel[x]->n_users; ++z) {
                          ...
                          channel[x]->channel_data->user_lists->addItem(
                              username);
                        }
                        y = channel[x]->n_users;
                        x = n_channels;
            ...
            case SERVER_MESSAGE_CHANNEL_JOIN:
              if (remaining_length > sizeof(channel_info)) {
                ...
                Channel **ptr =
                    (Channel **)malloc(sizeof(Channel *) * (n_channels + 1));
                ...
                memcpy(channel[n_channels]->channel_name,
                       new_channel.channel_name,
                       sizeof(new_channel.channel_name));
        ...
        if (0) {  // openssl tier memery - i'd write a separate function but i
                  // dont even knwo the context of this
...
void MainWindow::clear_chat() { ui->status_browser->clearHistory(); }
