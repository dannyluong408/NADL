...
private:
	...
	void command(const int session_id, unsigned char *msg, const int msglen);
	...
	void add_channel(const char *name, const uint8_t flags, const uint16_t id, const uint32_t owner_id, const uint32_t number_of_games, const uint8_t game_id);
	...
	int msg_channel_as_admin(const char *msg, const uint16_t len, const uint32_t sender, const uint16_t channel);
